...
- `evaluation/reports/latest.json` 已按 0.1.40 重新生成，覆盖 50 题 CodeGraph 策略，报告元数据为 `available=true`、`adapter=codegraph-public-cli`。
...
当前 CodeGraph Adapter 已接入且可用；0.1.40 活动报告的文件召回率为 `0.798333`、核心文件精确率为 `0.324667`、符号召回率为 `0.183333`、符号精确率为 `0.127381`、成功率为 `0.08`、P95 延迟为 `5860.27 ms`。冻结质量门仍未通过，这是检索质量问题，不是 Adapter 不可用，也不应通过恢复 Builtin fallback 或降低阈值解决。
