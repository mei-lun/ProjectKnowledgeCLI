...
- 状态检查将完整文件哈希、Git 提交对齐和 SQLite 健康状态作为独立维度读取。`content_fresh` 表示索引内容与工作区一致，`commit_aligned` 表示这些内容已经在当前 HEAD 上完成校验；纯提交变化可通过同步刷新元数据。文件系统与 Git 工作可并行处理，但不会削弱新鲜度检查。
