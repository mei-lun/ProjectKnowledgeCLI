...
上述两份文档从 0.1.21 起取代本报告原“0.1 最小需求收敛决议”，成为 0.1.22 功能开发的直接基线。下方 0.1 节保留为 0.1.15 阶段的历史决议，不再约束当前功能范围。
...
gardenserver 的 CodeGraph 1.5.0 事实快照与业务文件指纹核验已完成，初始化批次可恢复，代码变化只生成第二层项目事实指导草稿。当前 `.project-kb` 保留 5 个业务类别的独立方法论/项目事实指导待审核文件（共 10 份）；旧混合草稿未被复用。正式版本确认仍必须由用户分别审核两类资产，因此本项在用户确认前保持“待审核”，不能标记为最终完成。
