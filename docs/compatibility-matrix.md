...
| Windows 原生 | 部分支持 | Windows 绝对路径会被 EvidencePack 明确拒绝，避免越过项目边界 | watcher、Git hooks、文件锁和安装路径仍需 Windows CI 实机复验 |
...
| --- | --- | --- | --- | --- |
| Codex | `AGENTS.md`、`.project-kb/mcp.json`、Codex 插件 | 默认安装 | 只维护 `project-kb:instructions` 区块；插件版本与核心版本同步 | 支持 |
