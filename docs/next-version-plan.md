...
# 历史实施入口：0.1.32 → 0.1.33

0.1.33 已完成 RT-003 基础向量检索。下一批优先处理检索质量门中仍未达标的候选覆盖、Markdown 来源质量和精确率；不得降低冻结阈值，也不得让向量相似度覆盖路径、符号或 CodeGraph 结构证据。

后续向量增强必须覆盖：可选 local provider 的离线质量对照、索引成本、混合检索可复现、hybrid precision 不低于 lexical 基线，以及向量成本与 fallback 统计；网络 provider 仍不属于默认交付范围。
