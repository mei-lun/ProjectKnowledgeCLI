...
The result validates deterministic symbol-first ranking on the frozen gardenserver snapshot, not the production gate. The original plan's `precision@5` is now measured over the final Top-5 files against independently reviewed core plus acceptable-supporting labels; `core_file_precision` remains a stricter tier-width diagnostic and is not substituted for that metric. On the same 32 verified gardenserver questions, the exact/path-heavy Phase 0 seed reports `precision@5=0.45`, while the 20-question Phase 1 practice set reaches its `0.50` threshold. File, core-file, and symbol recall are `1.00` on both sets, and nDCG@5 is `0.938488/0.832547`.

Request-scoped read-only CodeGraph command reuse, snapshot/status reuse, exact query deduplication, and bounded high-confidence graph anchors reduce hybrid P95 from the earlier `19.93s/16.67s` to `4.26s/4.48s` on Phase 0/1 without lowering recall. The versioned Phase 1 threshold gate passes recall, `precision@5`, nDCG@5, and fallback checks; its only failure is P95 above the original `<1.5s` formal performance target. This is sufficient for a controlled gardenserver practice run, not the formal gate. The remaining process-startup cost requires a supported upstream batch/daemon or in-process interface; further blind anchor reduction is not accepted because it risks recall. The dataset still has only 32 questions from one stable snapshot, so RQ-P2-007/RQ-P3-005 production status remains pending. Optional model reranking is deliberately left pending until an offline provider and independent samples demonstrate a real gain without weakening local-first behavior.
