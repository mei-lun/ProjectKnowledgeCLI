...
| RQ-P3-004 | Persist complete debug traces and publish separate lexical, CodeGraph, ranking, and context-assembly P50/P95/P99 measurements | Trace schema v2 now records stage durations/status, context assembly trim events, context status, and evaluator stage percentiles; durable multi-snapshot performance evidence still required | partial |
