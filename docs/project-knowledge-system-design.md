...
按重新对齐后的第一阶段范围，项目已接入已安装的 CodeGraph 公开 CLI，并在 gardenserver 上验证了初始化、增量同步、事实查询和三类中文类别级开发指导。PKS 自有生成文件统一位于 `.project-kb/`；CodeGraph 1.5 的运行时索引仍必须位于项目根 `.codegraph/`，这是上游目录发现限制，PKS 不读取其私有数据库。

指导文档分为两层：第一层是可迁移的方法论，第二层是当前 gardenserver 的 Lua/Skynet/zn 适配事实。当前默认生成普通活动开发、普通玩家功能开发、登录模块开发三份中文文档；登录、花园、公会仅作为事实样本，不是最终知识粒度。
