...
报告按适用样本分别计算文件、符号、调用路径、扩展点、不变量和设计原因的召回率；结构锚点同时计算精确率。`precision_at_5` 使用最终预算后有序返回的前 5 个文件，相关集合为 `expected_files ∪ acceptable_supporting_files`，固定以 5 为分母；少于 5 个返回时空位按未命中计算，第 6 位以后不计入。`core_file_precision` 只认 `expected_files`，用于诊断 Core 分层是否过宽，不替代原方案的 Top-5 相关性指标。成本指标包括上下文 Token、工具调用次数、P50/P95/P99 延迟和最终成功率。报告还记录数据集哈希、项目提交、索引提交、引擎能力、Python、平台、处理器和 CPU 数。

空期望项不进入该指标分母，避免把“本题不要求符号”错误计算为符号召回失败。样本只有在其所有适用期望项均命中时才计为成功。

`acceptable_supporting_files` 必须由源码、调用关系、入口或测试关联人工确认；确认没有辅助相关文件时显式写 `[]`。它不得与 `expected_files` 重叠，也不得为了通过门槛事后扩张。gardenserver 的 32 条实践题已补齐该字段；12 条精确入口/路径种子题单独报告时不作为 `precision@5 >= 0.50` 的充分样本，实践判断使用 32 条合并结果。
