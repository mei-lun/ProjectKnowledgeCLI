...
class ProjectService:
    def __init__(
        ...
        path: str | Path = ".",
        *,
        engine_factory: Callable[[ProjectConfig], CodeIndexEngine] = create_engine,
    ):
        ...
        self.engine: CodeIndexEngine = self._engine_factory(self.config)
    ...
    def initialize(self, dry_run: bool = False) -> dict[str, Any]:
        if dry_run:
            ...
            return {
                ...
                "files_to_index": len(snapshot.files),
                "files_to_create": [
    ...
    def rebuild(self, dry_run: bool = False) -> dict[str, Any]:
        ...
        snapshot = self.engine.snapshot(self.root, self.config)
        if dry_run:
            return {"action": "rebuild", "dry_run": True, "files_to_index": len(snapshot.files)}
        with project_lock(self.root):
            result = self._atomic_rebuild(snapshot)
    ...
    def _atomic_rebuild(self, snapshot: CodeIndexSnapshot) -> dict[str, Any]:
        ...
        try:
            ...
            report = self._report(counts, list(snapshot.files), len(records), duration)
    ...
    def sync(self, dry_run: bool = False, task_summary: str = "manual synchronization") -> dict[str, Any]:
        self._require_initialized()
        started = time.monotonic()
        codegraph_sync = self.engine.sync(self.root, self.config) if not dry_run else None
        snapshot = self.engine.snapshot(self.root, self.config)
    ...
    def status(self) -> dict[str, Any]:
        ...
        freshness_unknown = not bool(engine_status.get("available"))
        content_fresh = not pending and not freshness_unknown
